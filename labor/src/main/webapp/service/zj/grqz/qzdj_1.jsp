...
<script>
	$(function(){
		...
		//通过身份证号判断年龄、性别
		var regx=/^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X)$/
		$("#sfzhm").blur(function(){
			if(regx.test($("#sfzhm").val())){
				
				if (parseInt($("#sfzhm").val().substr(16, 1)) % 2 == 1) {
					   $("#xb").load("../../sex/1")
					}else{
						$("#xb").load("../../sex/2")
					}
			...
			var Age = myDate.getFullYear() - $("#sfzhm").val().substring(6, 10)-1
			if ($("#sfzhm").val().substring(10, 12) < month || $("#sfzhm").val().substring(10, 12) == month && $("#sfzhm").val().substring(12, 14) <= day) {
				Age++;
			}
			$("#nl").val(Age)
		...
		$("#dwszs").change(function(){
			$("#dwszj").html("")
			$("#dwszq").load("../../city/"+$("#dwszs").val()+"/city")	

			 $("#dwszq").change(function(){
			$("#dwszj").load("../../city/"+$('#dwszq').val()+"/village")
		})
		   
		})
		//文化程度
		$("#whcd1").load("../../education")
		
		
	//	$("#init_zyjn").load("../../skill/)
		$("#init_jsdj").load("../../qualification")
		$("#init_csnx").load("../../")
		...
		$("#init_jywy").load("../../language")
		$("#init_wyslcd").load("../../proficiency")
		$("#jsjdj").load("../../computer")
		
		$("#jsjslcd").load("../../proficiency")
